...
def write_preset(name, test, value, work, reference=False):
    ...
    if value is not None:
        params[test["candidate"]["kparam"]] = value
    spec = SPEC0
    ...
    data = apply_spec(BASE.data, spec.model_copy(update={"fx_chain": [fx]}))
...
def run_test(sess, test, cfg, work):
    ...
    bpath = write_preset(test["id"] + "_base", test, None, work, reference=True)
    sess.load(bpath)
    ...
    for v, probe in [(v, False) for v in test["values"]] + [(v, True) for v in test.get("probe_values", [])]:
        p = write_preset("%s_%s" % (test["id"], key_of(v)), test, v, work)
        hashes[key_of(v)] = sha(p)
        sess.load(p)
        sv, bands, hosts = state_value(sess.state(), test), sess.bands(), sess.hosts()
        obs[key_of(v)] = {"written": v, "state_value": sv, "state_retained": same(sv, v) or (sv is None and dflt is not None and v == dflt), "probe": probe, "band_db": bands,
    ...
    rec = {"id": test["id"], "atlas_id": test.get("atlas_id"), "candidate": test["candidate"], "unit": test["unit"],
           ...
           "expect": test.get("expect", []), "preset_sha256": hashes,
